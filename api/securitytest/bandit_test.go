...
func loadBanditFixture(t *testing.T, name string) string {
	t.Helper()
	path := filepath.Join("testdata", "bandit", name)
...
// TestParseBandit_HighVulnerabilityFound asserts that a realistic bandit JSON
// output with HIGH and MEDIUM findings is parsed and bucketed correctly into
// scan.Vulnerabilities by severity.
func TestParseBandit_HighVulnerabilityFound(t *testing.T) {
	t.Parallel()

	defer func() {
		if r := recover(); r != nil {
			t.Fatalf("analyzeBandit panicked on valid input: %v", r)
		}
	}()
	...
	scan := &SecTestScanInfo{
		...
		Container: types.Container{
			COutput: loadBanditFixture(t, "high_vuln.json"),
		},
	}

	if err := analyzeBandit(scan); err != nil {
		t.Fatalf("analyzeBandit returned unexpected error: %v", err)
	}
...
// TestParseBandit_CleanOutputProducesEmptyResults asserts that a clean bandit
// run (no findings) yields no vulnerabilities and no error.
func TestParseBandit_CleanOutputProducesEmptyResults(t *testing.T) {
	t.Parallel()

	scan := &SecTestScanInfo{
		SecurityTestName: "bandit",
		Container: types.Container{
			COutput: loadBanditFixture(t, "no_vuln.json"),
		},
	}

	if err := analyzeBandit(scan); err != nil {
		t.Fatalf("analyzeBandit returned unexpected error on clean output: %v", err)
	}
...
// TestParseBandit_MalformedOutputDoesNotPanic catches gap #7 at the parser
...
func TestParseBandit_MalformedOutputDoesNotPanic(t *testing.T) {
	t.Parallel()

	defer func() {
		if r := recover(); r != nil {
			t.Errorf("bandit parser panicked on malformed input: %v\n"+
	...
	scan := &SecTestScanInfo{
		SecurityTestName: "bandit",
		Container: types.Container{
			COutput: loadBanditFixture(t, "malformed.json"),
		},
	}

	err := analyzeBandit(scan)
